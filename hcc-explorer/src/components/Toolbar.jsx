...
export function AnalysisToolbar({
  ...
  const safeHighlightColor = highlightColor || "#FFD600";

  return (
    <div style={{
      background: "#fff", borderBottom: "1px solid #e5e5e5",
      padding: "0 24px", display: "flex", alignItems: "center",
      height: 40, gap: 4, flexWrap: "nowrap", overflow: "hidden",
      flexShrink: 0, zIndex: 198,
    }}>

      {/* Different Words — disabled hint when only 1 col/row */}
      <ToolbarBtn
        active={showDiff}
        onClick={() => canUseDiff && setShowDiff(d => !d)}
        title={canUseDiff ? "Highlight words unique to this response vs peers" : "Need 2+ columns or rows to compare differences"}
        style={{ opacity: canUseDiff ? 1 : 0.4, cursor: canUseDiff ? "pointer" : "default" }}
      >
        Δ Words
      </ToolbarBtn>
      {showDiff && canUseDiff && (
      ...
      >
        <svg width="16" height="16" viewBox="0 0 24 24" fill="none">
          <rect x="5" y="3" width="14" height="10" rx="2"
            fill={highlightStyle === "fill" ? safeHighlightColor : "#ddd"}
      ...
      >
        <svg width="16" height="16" viewBox="0 0 24 24" fill="none">
          <text x="4" y="16" fontSize="14" fontWeight="bold"
            ...
            fontFamily="serif">U</text>
          <line x1="4" y1="20" x2="20" y2="20"
            stroke={highlightStyle === "underline" ? safeHighlightColor : "#bbb"}
          ...
          <div style={{
            ...
            {HIGHLIGHT_COLORS.map(c => (
              <button key={c} onClick={() => { setHighlightColor(c); setHlColorOpen(false); }} style={{
                ...
                border: c === safeHighlightColor ? "2.5px solid #111" : "1.5px solid rgba(0,0,0,.1)",
                cursor: "pointer", padding: 0,
              }}/>
            ))}
          </div>
      ...
      {/* Font size — big A, number, small A — side by side */}
      <div style={{ display: "inline-flex", alignItems: "center", gap: 1, marginLeft: 4 }}>
        <button onClick={() => setFontSize(s => Math.min(20, s + 1))} title="Increase font size"
          ...
          <span style={{ fontSize: 15, fontWeight: 700, fontFamily: "serif", lineHeight: 1 }}>A</span>
        </button>
        <span style={{ fontSize: 11, color: "#888", minWidth: 18, textAlign: "center", lineHeight: 1 }}>
          {fontSize}
        </span>
        <button onClick={() => setFontSize(s => Math.max(10, s - 1))} title="Decrease font size"
          ...
          <span style={{ fontSize: 10, fontWeight: 700, fontFamily: "serif", lineHeight: 1 }}>A</span>
        </button>
      </div>

      <Divider/>

      {/* Model color swatches — labeled "Display" */}
      <span style={{ fontSize: 10, color: "#bbb", letterSpacing: 0.5, textTransform: "uppercase", marginRight: 2 }}>
        Color
      </span>
      {allModels.map(m => (
        <ModelColorPicker
