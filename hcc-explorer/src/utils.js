...
export function isCrossLanguageDiff(rowDim, colDim, selLangs) {
  // Only warn when:
  // 1. More than one language is actually selected
  // 2. Language is NOT on any axis (so same-language peers aren't guaranteed)
  // When rowDim or colDim = "language", each row/col is exclusively one language,
  // so peers are always same-language. No inflation. No warning.
  // When only English OR only Spanish selected, no cross-language mixing. No warning.
  const multipleLanguagesSelected = selLangs.length > 1;
  const languageIsSeparated = rowDim === "language" || colDim === "language";
  return multipleLanguagesSelected && !languageIsSeparated;
}
